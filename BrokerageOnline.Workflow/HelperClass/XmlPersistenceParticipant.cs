...
namespace BrokerageOnline.Workflow
{
    public class XmlPersistenceParticipant : PersistenceParticipant
    {
        ...
        protected override IDictionary<XName, object> MapValues(IDictionary<XName, object> readWriteValues, IDictionary<XName, object> writeOnlyValues)
        {
            ...
            /**
            RackRateRequest requestForProposal = null;
            ...
            foreach (KeyValuePair<System.Xml.Linq.XName, object> item in writeOnlyValues)
            {
                if (item.Value is LocationInfo)
                {
                    ...
                    if (li.Value is RackRateRequest)
                    {
                        requestForProposal = (RackRateRequest)li.Value;
                    }
                }
            }
        ...
        XElement SerializeRfp(Rules_CreateRackRate rfp, Guid instanceId)
        {
            XElement ret =
               new XElement("rackRateRequest",
