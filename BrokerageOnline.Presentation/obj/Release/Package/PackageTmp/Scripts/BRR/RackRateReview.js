            ...
            if (gridData.length > 0) {
                for (var i = 0; i < gridData.length; i++) {
                    if (gridData[i].selectcheck == 'True') {
                        ...
                        var sendto = "";
                        Utility.ServiceCall("POST", 'BaseRackRateService.svc/SendEmailMemo', JSON.stringify({ fileurl: fileurl, filename: filename, sendto: sendto, typeval: 1, ModuleID: 1, MailStatus: "Distributor Mail BRR", sendbcc: BCCusers }), "json", false, false, function (result) {
...
$(function () {
    ...
    jQuery("#grid_search_result").jqGrid({
        ...
        sortable: true,
        rowNum: -1,
        colNames: ['Select', '<input type="checkbox" id="checkAll" >Select</input>', 'Memo Number', 'Memo Number', 'PaymentMemoId', 'Memo Type', 'Memo Type ID', 'DistributorCategoryId', 'Category', 'ARN No', 'ARN Name', 'From Date', 'To Date', 'Status', 'Status', 'Raised By', 'Raised On', 'Time', 'Ageing', 'Last Action By', 'Pending With', 'MemoLevel'],
        colModel: [
                ...
                { name: 'MemoId', index: 'MemoId', align: 'right', formatter: RackRate.ReturnSearchHyperLink },
